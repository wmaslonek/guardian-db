...
│  │    node_id -> ConnectionInfo                   │     │
│  └────────────────────────────────────────────────┘     │
│  ┌────────────────────────────────────────────────┐     │
│  │    Connection Pool (HashMap)                   │     │
│  │    node_id -> Vec<PooledConnection>            │     │
│  └────────────────────────────────────────────────┘     │
│  ┌────────────────────────────────────────────────┐     │
│  │    Circuit Breakers (HashMap)                  │     │
│  │    node_id -> CircuitBreaker                   │     │
│  └────────────────────────────────────────────────┘     │
│  ┌────────────────────────────────────────────────┐     │
│  │    Health Monitor                              │     │
│  │    - Peer health metrics                       │     │
...
```
get_connection(node_id, address)
    ↓
...
```rust
pub async fn get_connection(&self, node_id: PeerId, address: Multiaddr) -> Result<String>
```
...
let node_id = "12D3KooW...".parse()?;
let address: Multiaddr = "/ip4/192.168.1.100/tcp/4001".parse()?;

let connection_id = pool.get_connection(node_id, address).await?;
...
```rust
async fn try_reuse_connection(&self, node_id: PeerId) -> Result<Option<String>> {
...
async fn establish_new_connection(
    ...
    match connection_result {
        Ok(Ok(latency_ms)) => {
            ...
            self.active_connections.write().await.insert(node_id, connection_info);
...
```rust
pub async fn release_connection(&self, node_id: PeerId, connection_id: String) -> Result<()>
```
...
pool.release_connection(node_id, connection_id).await?;
```
...
Client                                  Peer
  ...
  ├─────────────────────────────────────>│
  │  [len][protocol][node_id][timestamp]  │
  │                                       │
...
async fn perform_connection_handshake(
    ...
    handshake_msg.extend_from_slice(&node_id.to_bytes());
...
```rust
async fn check_circuit_breaker(&self, node_id: PeerId) -> Result<bool> {
...
```rust
async fn record_failure(&self, node_id: PeerId) {
    let mut breakers = self.circuit_breakers.write().await;
    
    let breaker = breakers.entry(node_id).or_insert_with(CircuitBreaker::default);
    ...
    if breaker.failure_count >= breaker.failure_threshold
        ...
        self.event_sender.send(ConnectionEvent::CircuitBreakerOpen { node_id });
...
```rust
async fn record_success(&self, node_id: PeerId) {
    let mut breakers = self.circuit_breakers.write().await;
        ...
        match breaker.state {
            CircuitState::HalfOpen => {
                ...
                if breaker.success_count >= 3 {
                    ...
                    self.event_sender.send(ConnectionEvent::CircuitBreakerClosed { node_id });
                }
            },
            CircuitState::Closed => {
                // Reset failure count
...
pub enum ConnectionEvent {
    ...
    Connected { node_id: PeerId, latency_ms: f64 },
    
    /// Connection lost
    Disconnected { node_id: PeerId, reason: String },
    
    /// Connection degraded
    Degraded { node_id: PeerId, health_score: f64 },
    
    /// Connection recovered
    Recovered { node_id: PeerId },
    
    /// Circuit breaker opened
    CircuitBreakerOpen { node_id: PeerId },
    
    /// Circuit breaker closed
    CircuitBreakerClosed { node_id: PeerId },
}
```
...
tokio::spawn(async move {
    while let Ok(event) = event_stream.recv().await {
        match event {
            ConnectionEvent::Connected { node_id, latency_ms } => {
                println!("Connected to {} ({}ms)", node_id, latency_ms);
            },
            ConnectionEvent::CircuitBreakerOpen { node_id } => {
                warn!("Circuit breaker opened for {}", node_id);
            },
            ConnectionEvent::Degraded { node_id, health_score } => {
                warn!("Connection degraded: {} (score: {:.2})", node_id, health_score);
            },
            _ => {}
        }
    }
...
// First connection
let conn1 = pool.get_connection(node_id, address.clone()).await?;
println!("First connection: {}", conn1);
pool.release_connection(node_id, conn1).await?;

// Second connection (reuses first)
let conn2 = pool.get_connection(node_id, address.clone()).await?;
...
tokio::spawn(async move {
    while let Ok(event) = events.recv().await {
        match event {
            ConnectionEvent::Connected { node_id, latency_ms } => {
                info!("New connection: {} ({}ms)", node_id, latency_ms);
            },
            ConnectionEvent::Degraded { node_id, health_score } => {
                warn!("Degraded: {} (score: {:.2})", node_id, health_score);
            },
            ConnectionEvent::CircuitBreakerOpen { node_id } => {
                error!("Circuit breaker opened for {}", node_id);
            },
            ConnectionEvent::Recovered { node_id } => {
                info!("Connection recovered: {}", node_id);
            },
            _ => {}
        }
    }
...
tokio::spawn(async move {
    while let Ok(event) = events.recv().await {
        if let ConnectionEvent::CircuitBreakerOpen { node_id } = event {
...
async fn test_connection_pool_reuse() {
    let pool = OptimizedConnectionPool::new(PoolConfig::default());
    let node_id: PeerId = test_node_id();
    let address: Multiaddr = test_multiaddr();
    
    // First connection
    let conn1 = pool.get_connection(node_id, address.clone()).await.unwrap();
    pool.release_connection(node_id, conn1).await.unwrap();
    
    // Second connection (should reuse)
    let conn2 = pool.get_connection(node_id, address).await.unwrap();
...
async fn test_circuit_breaker() {
    ...
    let can_connect = pool.check_circuit_breaker(node_id).await.unwrap();
