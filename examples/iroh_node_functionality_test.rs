...
async fn main() -> Result<()> {
    ...
    info!("Testando funcionalidade do nó Iroh embarcado");

    // === TESTE 1: INICIALIZAÇÃO DO NÓ ===
    info!("TESTE 1: Inicialização do backend Iroh");

    let temp_dir = TempDir::new("iroh_test")
    ...
    if retrieved_data == test_data {
        ...
        return Err(guardian_db::error::GuardianError::Other(
    ...
    let backend2 = Arc::new(IrohBackend::new(&config).await?);
    sleep(Duration::from_secs(1)).await;
    ...
    match backend2.cat(&add_response.hash).await {
        Ok(mut stream) => {
            ...
            if data == test_data {
                ...
                info!("⚠️ Persistência parcial: dados diferem");
            }
        }
        Err(e) => {
            info!("⚠️ Persistência não detectada: {}", e);
        }
    }
...
mod tests {
    ...
    async fn test_iroh_node_basic_functionality() {
        let temp_dir = TempDir::new("iroh_basic_test").unwrap();
        let config = ClientConfig {
            data_store_path: Some(temp_dir.path().to_path_buf()),
            ..Default::default()
        };
