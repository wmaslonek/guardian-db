...
use crate::p2p::pubsub::PROTOCOL;
use crate::p2p::pubsub::direct_channel::DirectChannelNetwork;
use libp2p::{
    PeerId, SwarmBuilder,
...
impl SwarmManager {
    ...
    pub async fn configure_swarm(&self, local_peer_id: PeerId) -> Result<()> {
        ...
        // 2. Criação do Swarm com SwarmBridge
        let swarm_config = self.create_behaviour().await?;
        tracing::info!("Behaviour criado com SwarmBridge integrado");
    ...
    /// Cria behaviour com SwarmBridge otimizado
    async fn create_behaviour(&self) -> Result<String> {
        tracing::debug!("Criando behaviour com SwarmBridge...");

        // Cria nova instância da SwarmBridge
        let gossipsub_interface =
            crate::p2p::pubsub::direct_channel::SwarmBridge::new(self.span.clone()).await?;

        // Configura tópicos padrão
        ...
        let test_result = self
            .test_behaviour_functionality(&gossipsub_interface)
            .await?;
        ...
        let behaviour_stats = format!(
            "SwarmBridge[peer={}, protocol=Gossipsub] - Config: validation=strict, heartbeat=1s, max_size=4MB, history=5, topics={}",
            local_peer_id,
            default_topics.len()
        );

        tracing::info!("SwarmBridge criado com sucesso: {}", behaviour_stats);
    ...
    async fn test_behaviour_functionality(
        &self,
        gossipsub_interface: &crate::p2p::pubsub::direct_channel::SwarmBridge,
    ) -> Result<String> {
        tracing::debug!("Testando funcionalidade do behaviour...");

        // Funcionalidade agora testada via SwarmBridge
        let test_topic = format!("{}/test", PROTOCOL);
        let test_message = b"behaviour_test_message".to_vec();
        let test_topic_hash = libp2p::gossipsub::IdentTopic::new(&test_topic).hash();
        gossipsub_interface
            .publish_message(&test_topic_hash, &test_message)
            .map_err(|e| GuardianError::Other(format!("Erro testar publicação: {}", e)))?;

        // Verifica peers conectados
        let connected_peers = gossipsub_interface.get_connected_peers();

        // Testa inscrição em tópico adicional
        let test_topic_hash = libp2p::gossipsub::IdentTopic::new(&test_topic).hash();
        gossipsub_interface
            .subscribe_topic(&test_topic_hash)
    ...
    async fn validate_behaviour(&self, behaviour_stats: &str) -> Result<()> {
        ...
        // Validações básicas
        if !behaviour_stats.contains("SwarmBridge") {
            return Err(GuardianError::Other(
