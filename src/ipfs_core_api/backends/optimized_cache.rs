...
/// Cache Layer otimizado para operações IPFS/Iroh
...
impl OptimizedCache {
    ...
    pub fn new(config: CacheConfig) -> Self {
        let data_cache_size = NonZeroUsize::new(config.max_data_entries)
            .unwrap_or(NonZeroUsize::new(10_000).unwrap());
        let compressed_cache_size = NonZeroUsize::new(config.max_compressed_entries)
    ...
    pub async fn get(&self, cid: &str) -> Option<Bytes> {
        ...
        if self.config.enable_access_prediction {
    ...
    pub async fn put(&self, cid: &str, data: Bytes) -> Result<()> {
        ...
        let should_compress = data_size >= self.config.compression_threshold;
    ...
    async fn check_and_evict(&self) -> Result<()> {
        ...
        let max_usage =
            (self.config.max_data_cache_size + self.config.max_compressed_cache_size) as f64;

        if current_usage / max_usage > self.config.eviction_threshold {
