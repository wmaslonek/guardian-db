...
pub async fn create_db_manifest(
    ipfs: &crate::kubo_core_api::IpfsClient,
    name: &str,
    db_type: &str,
    ...
    let manifest = Manifest {
        ...
        access_controller: access_controller_path.as_path().to_string_lossy().into_owned(),
    };

    // Serializa o manifesto para CBOR
    let cbor_data = serde_cbor::to_vec(&manifest)
        .map_err(|e| GuardianError::Other(format!("Não foi possível escrever os dados do manifesto em CBOR: {}", e)))?;
    ...
    let cid: Cid = response.hash.parse()
...
pub async fn read_db_manifest(ipfs: &crate::kubo_core_api::IpfsClient, manifest_cid: &Cid) -> Result<Manifest> {
    // Busca os dados do manifesto no IPFS usando cat
    let mut stream = ipfs.cat(&manifest_cid.to_string()).await
        .map_err(|e| GuardianError::Other(format!("Não foi possível buscar o manifesto no IPFS: {}", e)))?;
    ...
    let manifest: Manifest = serde_cbor::from_slice(&data)
        .map_err(|e| GuardianError::Other(format!("Não foi possível decodificar o manifesto CBOR: {}", e)))?;
