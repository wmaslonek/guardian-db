...
use crate::eqlabs_ipfs_log::entry::EntryOrHash;
...
impl<'a> LogOptions<'a> {
	...
	where F: 'static + Fn(&Entry,&Entry) -> Ordering + Send + Sync {
...
impl Log {
	...
	pub fn new (ipfs: Arc<IpfsClient>, identity: Identity, opts: LogOptions) -> Log {
		...
		(opts.id, opts.access, opts.entries, opts.heads, opts.clock, opts.sort_fn);
		let sort_fn = Box::new(Entry::no_zeroes(sort_fn.unwrap_or(Box::new(Entry::last_write_wins))));
	...
	pub fn from_multihash (ipfs: Arc<IpfsClient>, identity: Identity, opts: LogOptions, hash: &str) -> Log {
		let es = Entry::fetch_entries(&ipfs,&[hash.to_owned()]).into_iter().map(|x| Arc::new(x)).collect::<Vec<Arc<Entry>>>();
	...
	pub fn append (&mut self, data: &str, n_ptr: Option<usize>) -> &Entry {
		...
		let hash_result = Runtime::new().unwrap().block_on(Entry::multihash(&self.ipfs,&entry)).unwrap();
	...
	pub fn join (&mut self, other: &Log, size: Option<usize>) -> Option<&Log> {
		...
		let merged_heads: Vec<Arc<Entry>> = all_heads.into_iter().filter(|x| !nexts_from_new_items.contains(&x.hash().to_owned())).
	...
	pub fn traverse (&self, roots: &[Arc<Entry>], amount: Option<usize>, end_hash: Option<String>) -> Vec<Arc<Entry>> {
