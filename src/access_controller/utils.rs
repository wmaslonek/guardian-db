...
pub async fn create(
    ...
    match controller_type_normalized.as_str() {
        ...
        _ => {
            ...
            return Err(GuardianError::Store(format!("Unknown access controller type: {}", controller_type)));
        }
    }

    // Cria o controlador baseado no tipo
    let controller = create_controller(&controller_type_normalized, params.clone(), logger.clone()).await?;
    ...
    match Cid::try_from(cid_str.as_str()) {
        ...
        Err(e) => {
            ...
            Err(GuardianError::Store(format!("Failed to create manifest CID: {}", e)))
        }
    }
}
...
async fn create_controller(
    ...
    match controller_type {
        ...
        _ => {
            ...
            Err(GuardianError::Store(format!("Unsupported access controller type: {}", controller_type)))
        }
    }
}
...
pub fn validate_address(address: &str) -> Result<()> {
    ...
    if address.contains("..") || address.contains("//") {
        return Err(GuardianError::Store("Address contains invalid path components".to_string()));
    }
    
    // Verifica comprimento máximo
    if address.len() > 1000 {
        return Err(GuardianError::Store("Address is too long (max 1000 characters)".to_string()));
    }
