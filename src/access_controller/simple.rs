...
impl SimpleAccessController {
    ...
    pub fn new(logger: slog::Logger, initial_keys: Option<HashMap<String, Vec<String>>>) -> Self {
        ...
        allowed_keys.entry("read".to_string()).or_insert_with(Vec::new);
        allowed_keys.entry("write".to_string()).or_insert_with(Vec::new);
        allowed_keys.entry("admin".to_string()).or_insert_with(Vec::new);
    ...
    pub async fn clear_capability(&self, capability: &str) -> Result<()> {
        if capability.is_empty() {
            return Err(GuardianError::Store("Capability cannot be empty".to_string()));
        }
    ...
    pub async fn grant_multiple(&self, capability: &str, key_ids: Vec<&str>) -> Result<()> {
        ...
        let mut state = self.state.write().await;
        let keys = state.allowed_keys.entry(capability.to_string()).or_insert_with(Vec::new);
    ...
    pub async fn clone_capability(&self, source_capability: &str, target_capability: &str) -> Result<()> {
        if source_capability.is_empty() || target_capability.is_empty() {
            return Err(GuardianError::Store("Source and target capabilities cannot be empty".to_string()));
        }
        ...
        if let Some(source_keys) = state.allowed_keys.get(source_capability) {
            ...
            state.allowed_keys.insert(target_capability.to_string(), cloned_keys);
            ...
            return Err(GuardianError::Store(format!("Source capability '{}' not found", source_capability)));
        }
...
impl AccessController for SimpleAccessController {
    ...
    async fn grant(&self, capability: &str, key_id: &str) -> Result<()> {
        ...
        let entry = state.allowed_keys.entry(capability.to_string()).or_insert_with(Vec::new);
    ...
    async fn can_append(
        ...
        if let Some(admin_keys) = state.allowed_keys.get("admin") {
            if admin_keys.contains(&"*".to_string()) || admin_keys.contains(&entry_id.to_string()) {
                ...
                if let Err(e) = identity_provider.verify_identity(entry_identity).await {
                    ...
                    return Err(GuardianError::Store(format!("Invalid identity signature for admin key {}: {}", entry_id, e)));
                }
        ...
        Err(GuardianError::Store(format!("Access denied: identity {} not authorized for write operations", entry_id)))
    }
}