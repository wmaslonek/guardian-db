...
//!
//! - [`EventBus`]: typed pub/sub over Tokio broadcast channels, keyed by event
//!   type (`TypeId`). The backbone used by the core, stores, access control
//!   and compute layers.
//! - [`EventEmitter`] / [`EmitterInterface`]: the type-erased facade stores
//!   expose via `events()` (see [`emitter`]).
//! - [`PayloadEmitter`]: bridges direct-channel payloads into the bus as
//!   [`EventPubSubPayload`] events.

use crate::guardian::error::{GuardianError, Result};
use crate::traits::{DirectChannelEmitter, EventPubSubPayload};
...
pub use emitter::{EmitterInterface, Event, EventBox, EventEmitter};
