...
pub async fn save_snapshot<S: Store + Send + Sync>(store: &S) -> Result<Cid> {
    ...
    let snapshot_header = StoreSnapshot {
        ...
        heads: oplog.heads().iter().map(|arc_entry| (**arc_entry).clone()).collect(),
        size: oplog.len(),
    ...
    for entry in oplog.values() {
        let entry_json = serde_json::to_vec(&*entry)
            .map_err(|e| GuardianError::Store(format!("Unable to serialize entry as JSON: {}", e)))?;
    ...
    let add_response = store.ipfs().add(Cursor::new(rs)).await
        .map_err(|e| GuardianError::Store(format!("Unable to save log data on store: {}", e)))?;
    ...
    cache.put("snapshot".as_bytes(), snapshot_cid.to_string().as_bytes()).await
        .map_err(|e| GuardianError::Store(format!("Unable to add snapshot data to cache: {}", e)))?;
    ...
    cache.put("queue".as_bytes(), &unfinished_json).await
        .map_err(|e| GuardianError::Store(format!("Unable to add unfinished data to cache: {}", e)))?;
