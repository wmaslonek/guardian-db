...
impl SledKeystore {
    ...
    pub fn new(path: Option<std::path::PathBuf>) -> Result<Self> {
        let db = match path {
            Some(p) => sled::open(p).map_err(|e| GuardianError::Other(format!("Erro ao abrir sled: {}", e).into()))?,
            None => sled::Config::new().temporary(true).open().map_err(|e| GuardianError::Other(format!("Erro ao criar sled temporário: {}", e).into()))?,
        };
    ...
    pub async fn put_keypair(&self, key: &str, keypair: &Keypair) -> Result<()> {
        let encoded = keypair.to_protobuf_encoding()
            .map_err(|e| GuardianError::Other(format!("Erro ao codificar keypair: {}", e).into()))?;
        ...
        match self.get(key).await? {
            Some(bytes) => {
                let keypair = Keypair::from_protobuf_encoding(&bytes)
                    .map_err(|e| GuardianError::Other(format!("Erro ao decodificar keypair: {}", e).into()))?;
                Ok(Some(keypair))
            }
            None => Ok(None),
        }
    ...
    pub async fn list_keys(&self) -> Result<Vec<String>> {
        let keys: std::result::Result<Vec<String>, sled::Error> = self.db
            .iter()
            .keys()
...
impl KeystoreInterface for SledKeystore {
    async fn put(&self, key: &str, value: &[u8]) -> Result<()> {
        self.db.insert(key, value)
            .map_err(|e| GuardianError::Other(format!("Erro ao inserir no keystore: {}", e).into()))?;
        Ok(())
    }

    async fn get(&self, key: &str) -> Result<Option<Vec<u8>>> {
        match self.db.get(key).map_err(|e| GuardianError::Other(format!("Erro ao recuperar do keystore: {}", e).into()))? {
    ...
    async fn has(&self, key: &str) -> Result<bool> {
        Ok(self.db.contains_key(key).map_err(|e| GuardianError::Other(format!("Erro ao verificar chave no keystore: {}", e).into()))?)
    }

    async fn delete(&self, key: &str) -> Result<()> {
        self.db.remove(key).map_err(|e| GuardianError::Other(format!("Erro ao remover do keystore: {}", e).into()))?;
        Ok(())
    }
}

/// Factory function para criar keystores baseados em configuração
pub fn create_keystore(directory: Option<std::path::PathBuf>) -> Result<Arc<dyn KeystoreInterface + Send + Sync>> {
...
mod tests {
    ...
    async fn test_keypair_storage() {
        ...
        // Store it
        keystore.put_keypair(key_name, &original_keypair).await.unwrap();
