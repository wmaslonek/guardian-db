...
use slog::{Logger, o, Discard, error, warn, debug};
use tokio_util::sync::CancellationToken;
use ipfs_api_backend_hyper::IpfsClient as HyperIpfsClient;
use libp2p::PeerId;
use opentelemetry::global::BoxedTracer;
use crate::eqlabs_ipfs_log::identity::Identity;
use crate::kubo_core_api::IpfsClient; // Our KuboCoreApiClient alias
use crate::iface::{
    AccessControllerConstructor, CreateDBOptions, DetermineAddressOptions, DirectChannel, DirectChannelFactory, DirectChannelOptions, EventPubSubPayload, MessageExchangeHeads, MessageMarshaler, PubSubInterface, Store, StoreConstructor
};
...
impl GuardianDB {
    ...
    pub async fn new(_ipfs: HyperIpfsClient, options: Option<NewGuardianDBOptions>) -> Result<Self> {
        ...
        let identity = if let Some(identity) = options.identity {
            ...
            let _id = options.id.as_deref().unwrap_or(&peer_id.to_string()).to_string();
            let _keystore = options.keystore.as_ref()
                .ok_or_else(|| GuardianError::Other("Keystore é necessário para criar uma identidade".to_string()))?;
    ...
    pub async fn new_orbit_db(
        ...
        let logger = options.logger.unwrap_or_else(|| Logger::root(Discard, o!()));
        let tracer = options.tracer.unwrap_or_else(|| {
            // Usar um placeholder simples para o tracer
            Arc::new(BoxedTracer::new(Box::new(opentelemetry::trace::noop::NoopTracer::new())))
        });
        ...
        let emitters = Emitters::generate_emitters(&event_bus).await
            .map_err(|e| GuardianError::Other(format!("Falha ao gerar emitters do EventBus: {}", e)))?;
        ...
        let directory = options.directory.unwrap_or_else(|| PathBuf::from("./GuardianDB/in-memory")); // Placeholder para InMemoryDirectory
        ...
        let odb = Arc::new_cyclic(|weak_self| {
            GuardianDB {
                ...
                this: weak_self.upgrade().unwrap_or_else(|| panic!("Failed to create Arc")),
                emitters,
    ...
    pub async fn close_all_stores(&self) {
        let stores_to_close: Vec<Arc<GuardianStore>> = self.stores.read().values().cloned().collect();
    ...
    pub fn get_access_controller_type(&self, controller_type: &str) -> Option<AccessControllerConstructor> {
        ...
        if self.access_controller_types.read().contains_key(controller_type) {
    ...
    pub async fn register_access_controller_type(&self, constructor: AccessControllerConstructor) -> Result<()> {
        ...
        let _dummy_options = crate::access_controller::manifest::CreateAccessControllerOptions::new_empty();
        ...
        if controller_type.is_empty() {
            return Err(GuardianError::InvalidArgument("o tipo do controller não pode ser uma string vazia".to_string()));
        }
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        slog::debug!(self.logger, "Emitters serão fechados automaticamente com o EventBus");
    ...
    pub async fn create(&self, name: &str, store_type: &str, options: Option<CreateDBOptions>) -> Result<Arc<GuardianStore>> {
        debug!(self.logger, "Create()");
        let options = options.unwrap_or_default();

        // O diretório pode ser passado como uma opção, caso contrário, usa o padrão da instância.
        let directory = options.directory.clone().unwrap_or_else(|| self.directory.to_string_lossy().to_string());
        ...
        let determine_opts = crate::iface::DetermineAddressOptions {
            only_hash: None,
            replicate: None,
            access_controller: crate::access_controller::manifest::CreateAccessControllerOptions::new_empty(),
        };
        let db_address = self.determine_address(name, store_type, Some(determine_opts)).await?;
        ...
        self.add_manifest_to_cache(&directory_path, &db_address).await
            .map_err(|e| GuardianError::Other(format!("não foi possível adicionar o manifesto ao cache: {}", e)))?;
    ...
    pub async fn open(&self, db_address: &str, options: CreateDBOptions) -> Result<Arc<GuardianStore>> {
        ...
        if crate::address::is_valid(db_address).is_err() {
            ...
            if store_type.is_empty() {
                ...
                return Err(GuardianError::InvalidArgument(format!("Tipo de banco de dados não fornecido! Forneça um tipo com 'options.store_type'. {}", types_list)));
            }
        ...
        if options.local_only.unwrap_or(false) && !self.have_local_data(&parsed_address).await {
            return Err(GuardianError::NotFound(format!("O banco de dados não existe localmente: {}", db_address)));
        }
        ...
        let manifest_type = if self.have_local_data(&parsed_address).await {
            ...
            debug!(self.logger, "Dados encontrados localmente, lendo manifesto do IPFS");
            let manifest = db_manifest::read_db_manifest(self.ipfs(), &parsed_address.get_root())
                .await
                .map_err(|e| GuardianError::Other(format!("Não foi possível ler o manifesto do IPFS: {}", e)))?;
            manifest.r#type
        } else {
            // Se não temos dados locais, lê diretamente do IPFS
            debug!(self.logger, "Dados não encontrados localmente, lendo manifesto do IPFS");
            let manifest = db_manifest::read_db_manifest(self.ipfs(), &parsed_address.get_root())
                .await
                .map_err(|e| GuardianError::Other(format!("Não foi possível ler o manifesto do IPFS: {}", e)))?;
            manifest.r#type
        ...
        self.create_store(&manifest_type, &parsed_address, options).await
    }
    ...
    pub async fn determine_address(
        ...
        if self.get_store_constructor(store_type).is_none() {
            let available_types = self.store_types_names();
            return Err(GuardianError::InvalidArgument(format!("tipo de banco de dados inválido: {}. Tipos disponíveis: {:?}", store_type, available_types)));
        }

        if crate::address::is_valid(name).is_ok() {
            return Err(GuardianError::InvalidArgument("o nome do banco de dados fornecido já é um endereço válido".to_string()));
        }

        // Cria opções padrão para o access controller
        let _ac_params = crate::access_controller::manifest::CreateAccessControllerOptions::new_empty();
        ...
        let manifest_hash = db_manifest::create_db_manifest(self.ipfs(), name, store_type, &ac_address_string)
            .await
            .map_err(|e| GuardianError::Other(format!("não foi possível salvar o manifesto no ipfs: {}", e)))?;
    ...
    pub async fn load_cache(&self, _directory: &PathBuf, db_address: &GuardianDBAddress) -> Result<()> {
    ...
    pub async fn add_manifest_to_cache(&self, directory: &PathBuf, db_address: &GuardianDBAddress) -> Result<()> {
        let _cache_key = format!("{}/_manifest", db_address.to_string());
    ...
    pub async fn monitor_direct_channel(&self, event_bus: Arc<EventBusImpl>) -> Result<JoinHandle<()>> {
        let mut receiver = event_bus.subscribe::<EventPubSubPayload>().await
            .map_err(|e| GuardianError::Other(format!("não foi possível se inscrever nos eventos do pubsub: {}", e)))?;
