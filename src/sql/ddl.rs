...
use crate::sql::store::{encode_row, Mutation};
        ...
        if ci.unique {
            if let Some(loaded) = self.tables.get(&q) {
                ...
                for (rid, values) in &loaded.rows {
                    let key = crate::relational::ordered_key(&crate::sql::store::index_values(idx, values));
                    if crate::relational::composite_key(&crate::sql::store::index_values(idx, values)).is_some() {
