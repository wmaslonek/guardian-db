...
use crate::sql::store::{derive_row_id, encode_row, index_values, Mutation, RowValues};
use crate::relational::catalog::{QualifiedName, Table};
use crate::relational::{composite_key, ordered_key, SqlType, SqlValue};
use sqlparser::ast::{
        ...
        for item in items {
            match item {
                ...
                SelectItem::UnnamedExpr(e) => {
                    fields.push(OutField::new(crate::sql::select::default_col_name(e), self.infer_type(e, &schema)));
                }
